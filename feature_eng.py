        ...
        for ex in examples:
            ...
            for i in range(len(words)):
                if re.search('\"',words[i]):
                    features[j]+=1
            j += 1
        print("QuoteTransformer: ",len(features))
        return features
...
class Featurizer:
    def __init__(self):
        ...
        self.all_features = FeatureUnion([
            ...
            ('punc_pipeline', Pipeline([
                ('punc',PunctutationTransformer()),
                ('scaledot',Normalizer())              
            ])),
            ('rating', Pipeline([
                ('rating_trans',RateTransformer()),
                ('scale_rating',Normalizer())              
            ])),
            ('text_stats', Pipeline([
    ...
    def train_feature(self, examples):
        print("inside train_feature to fit transform on train data")
        train_features = self.all_features.fit_transform(examples)
        print("completed fit transform on train data")
        return train_features

    def test_feature(self, examples):
        test_features= self.all_features.transform(examples)
        print("completed fit transform on test data")
...
if __name__ == "__main__":
    ...
    alphalist = [0.000001, 0.00001, 0.0001,0.001]
    scores=[]
